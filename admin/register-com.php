...
if (empty($name)) {
	...
	header('location:register.php?name_error');
}else if (empty($email)) {
	...
	header('location:register.php?valid_email_error');
}else if (empty($password)) {
	$_SESSION['pass_err'] = 'Password fild require...';
	header('location:register.php?password_fild_error');

}else if (strlen($password)<6) {
	$_SESSION['pass_err'] = '6 Charecter Password require...';
	header('location:register.php?password_not_valid');
}else if(empty($con_pass)){
	$_SESSION['con_pass_err'] = 'Confirm password require';
	header('location:register.php?Confirm_password_error');
}else if ($password != $con_pass) {
	$_SESSION['con_pass_err'] = 'Password Not Match';
	header('location:register.php?Confirm_password_not_matches');
}else if (empty($gender)) {
	$_SESSION['gen_err'] = 'Select Gender';
	header('location:register.php?gender_error');
}
// else if (empty($country)) {
// 	$_SESSION['country_err'] = 'Please Select Your Country';
// 	header('location:register.php?country_error');
// }
else{
	...
	if (in_array($extension, $allow_extension)) {
		if ($u_img['size'] <= 200000) {
			...
			if ($check_db_assoc['dupli']==1) {
				...
				header('location:register.php?duplicate_email_found');
			}else{
				...
				header('location:register.php?register_complete');
			}
		}else{
			$_SESSION['pic_err'] = 'Image size too large';
			header('location:register.php?picture_size_err');
		}
	}else{
		$_SESSION['pic_err'] = 'Image File Not Support';
		header('location:register.php?picture_not_support');
	}
